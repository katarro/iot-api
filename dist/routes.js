...
router.post("/api/v1/sensors", (req, res) => {
  ...
  db.query("INSERT INTO Sensor (location_id, sensor_name, sensor_category, sensor_meta, sensor_api_key) VALUES (?, ?, ?, ?, ?)", [location_id, sensor_name, sensor_category, sensor_meta, sensor_api_key], (err, result) => {
    if (err) {
      console.error("Database error:", err.message);
      return res.status(500).json({ error: "Database error", message: err.message });
    }
...
router.put("/api/v1/sensors/:id", (req, res) => {
  ...
  db.query("UPDATE Sensor SET location_id = ?, sensor_name = ?, sensor_category = ?, sensor_meta = ?, sensor_api_key = ? WHERE id = ?", [location_id, sensor_name, sensor_category, sensor_meta, sensor_api_key, id], (err, result) => {
    if (err) {
      console.error("Database error:", err.message);
      return res.status(500).json({ error: "Database error", message: err.message });
    }
